...
deploy() {
  ...
  IMAGE_DIR=$(mktemp -d /tmp/SCREENING-API.XXXXXX)
  ...
  IMAGE_S3_PATH='s3://polestar-images/screening-api/'
...
EOF
  ...
  ZIPNAME="screening-api-${APP_VERSION}.zip"
  ZIPPATH="/tmp/${ZIPNAME}"
  {
    cd ${IMAGE_DIR}
    zip -r ${ZIPPATH} .
  ...
  aws deploy create-deployment --application-name screening \
    --deployment-group-name test-screening-api \
    --s3-location bucket=polestar-images,key=screening-api/${ZIPNAME},bundleType=zip
}
