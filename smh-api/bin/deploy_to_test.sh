...
deploy() {
  ...
  IMAGE_DIR=$(mktemp -d /tmp/${PROJECT_NAME^^}.XXXXXX)
  DEVOPS_DIR=$(mktemp -d /tmp/DEVOPS.XXXXXX)
  CONFIG_DIR="${DEVOPS_DIR}/cloudformation/${PROJECT_NAME^^}/appspec"
  APP_VERSION=${VERSION}
  if [[ ! ${BUMPRELEASE} ]]; then
    APP_VERSION=${VERSION}-${REVISION}
  fi
  echo Detected ${DEVOPS_BRANCH}
  DEVOPS_BRANCH=${DEVOPS_BRANCH:-master}
  IMAGE_S3_PATH="s3://polestar-images/${PROJECT_NAME}/"
...
EOF
  ...
  rsync -av --progress ${APP_DIR}/ ${IMAGE_DIR}/${PROJECT_NAME}/ --exclude .git --exclude .gitignore --exclude images --exclude build --exclude devops-aws --exclude tests --exclude .idea --exclude .pytest_cache
  echo "${DEVOPS_BRANCH}" > ${IMAGE_DIR}/${PROJECT_NAME}/DEVOPS_BRANCH

  ZIPNAME="smh-${APP_VERSION}.zip"
  ZIPPATH="/tmp/${ZIPNAME}"
  {
    cd ${IMAGE_DIR}
    zip -r ${ZIPPATH} .
  ...
  aws deploy create-deployment --application-name ${PROJECT_NAME} --deployment-group-name test-${PROJECT_NAME} --s3-location bucket=polestar-images,key=${PROJECT_NAME}/${ZIPNAME},bundleType=zip
}
